...
function TrigSolver(inputIds) {
  ...
  /*
      Pass this constructor function an array of strings.  Each string must
      take the following form: <{feature}{label}>, where feature is either
      'angle' or 'length' and label is either 'a', 'b', or 'c'.
      
      Example: ['lengthA', 'lengthB', 'angleC']
 
      Get back an object that has two keys, one method and one property.
      The method will be used to generate a solution for the specified configuration
      and the property gives validation detail for the configuration.
  */
  ...
  const config = (function () {
    ...
    const myData = data.filter(item => item.scope.includes(caseId))[0];
    ...
    const caseValidation = (function () {
      ...
      const myRules = rules.filter(item => item.scope.includes(myData.name));
  ...
  this.solve = function (input) {
    ...
    return {
      "validation": config.validate(input),
      "solution": config.solve(input)
    }
